...
rm(list = ls()) #오브젝트 내의 모든 변수 삭제 또는 빗자루 버튼
...
rep(c(4,2), times=c(3,3)) # times 에 주어진 수 만큼 반복 



# 문제1 : 1부터 100까지 짝수를 2번씩 출력
rep(seq(2,100,2), each = 2)

# 문제 2
for(i in 1:5) {result[i] <- rep(i, times = i)}
result # 마지막 값만 나옮
...
#실습 1
#
A <-  rep(seq(1.0,3.0, 0.5),times = 1:5)
B <-  1:15

M <- matrix(data = c(B,A), ncol=5, byrow = T)
...
x.list <- list(c(41,25,35), matrix(10:17, nrow=4),c("Kim","Lee", "Cho"))
